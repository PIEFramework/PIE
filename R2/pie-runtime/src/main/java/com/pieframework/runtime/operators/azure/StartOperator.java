...
public class StartOperator extends AzureBaseOperator {

	public Status help(Component c,Request r){
		r.getStatus().addMessage("info", this.getClass().getSimpleName()+" starts the VMs that were setup by the 'create' command. The operator accepts -i slot='stage|production' which identifies the deployment target.");
		return null;
	}
	...
	public Status process(SubSystem c, Request r) {
		...
		// Check if all role vms are in stopped state
		Deployment deploy = sm.getDeployment(ahs.getUrlPrefix(), slotType);
		Boolean stopped = true;
		for (RoleInstance roleInst : deploy.getRoleInstances()) {
			if (roleInst.getInstanceStatus() != InstanceStatus.Stopped) {
				stopped = false;
				status
						.addMessage("info",
								"All roles must be in stopped state before they can be started.");
			}
		}
		...
		if (stopped) {
			...
			try {
				...
				result = sm.updateDeplymentStatus(ahs.getUrlPrefix(), slotType,
						UpdateStatus.Running, callback);
			} catch (Exception e) {
			...
			AzureUtils.getOperationStatus(result, pollPeriod, sm, status,
					"Start");

			// Verify that all are up
			Boolean startInProgress = true;
			int counter = 0;
			if (requestCompleted) {

				while (startInProgress && counter < timeout) {
					...
					Deployment dp = sm.getDeployment(hostedServiceName,
							slotType);

					for (RoleInstance roleInst : dp.getRoleInstances()) {
						if (roleInst.getInstanceStatus() != InstanceStatus.Ready) {
				...
				if (!startInProgress) {
					status
							.addMessage("info",
									this.getClass().getSimpleName()+" completed and all instances are up.");
				}
				...
				if (counter >= timeout) {
					status
							.addMessage(
									"warn",
									this.getClass().getSimpleName()+" is still in-progress after over "
											+ timeout
											* pollPeriod
											+ " seconds. PIE is disconnecting from the start operation, which will continue running.");
				}
			}
		}
